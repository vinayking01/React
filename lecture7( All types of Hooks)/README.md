...
## Q: Why should we never create state variables inside functions or conditional blocks in React?
Placing useState inside functions, if conditions, or loops breaks React’s Rules of Hooks and causes inconsistencies in state management. All state variables should be declared at the top level inside the component body, never outside or inside conditional logic.

## Rules
1. Always write hooks inside the component or function at the top of hierarchy.
2. Hooks can only be called at the top level of a component.
3. Hooks cannot be conditional.  Never call hooks inside loops, conditions, or nested functions.
    ```jsx
      const Example = ({ shouldUseHook }) => {
        if (shouldUseHook) {
          // ❌ React might not know where to assign this state on re-render
          const [count, setCount] = useState(0);
        }

        return <div>Invalid Hook Usage</div>;
      };

      🔴 Why is this wrong?

      If shouldUseHook is false during the first render, useState is never called.
      If shouldUseHook is true in the next render, useState appears at a new position, confusing React’s hook tracking.
      React expects hooks to be in the same order in every render.
...
### 🔥 What is the Return Statement Inside `useEffect`?
In React, the return statement inside `useEffect` is **only used for cleanup functions**.  
It **prevents memory leaks**, **removes unnecessary event listeners**, and **clears background tasks** when a component unmounts or when dependencies change.

---

### 🎯 What Problems Does It Solve?

| Issue | How Cleanup Solves It |
|-------|----------------------|
| **Memory Leaks** (e.g., event listeners or timers staying active after unmounting) | Removes them when the component unmounts. |
| **Duplicate Listeners** (e.g., multiple `addEventListener` calls stacking up) | Ensures old listeners are removed before new ones are added. |
| **Unnecessary API Calls** (e.g., making API requests even after unmounting) | Cancels pending API requests using `AbortController`. |
| **Multiple Timers Running** (e.g., `setInterval` keeps running even after unmounting) | Clears intervals to stop execution. |
| **WebSocket Connections Remaining Open** | Closes the WebSocket connection when the component unmounts. |

---

### ⏳ When Does the Cleanup Function Run?

1️⃣ **Before the component unmounts** → Common case (e.g., removing event listeners).  
2️⃣ **Before `useEffect` runs again (when dependencies change)** → Ensures the old effect is cleaned up before running a new one.

---

### ✅ Examples of Cleanup in `useEffect`

#### 1️⃣ Cleaning Up an Event Listener
```jsx
useEffect(() => {
  const handleResize = () => console.log("Resized!");
  window.addEventListener("resize", handleResize);

// The Below cleanup function in useEffect runs before the effect re-runs (if dependencies exist) or before the component unmounts. It does not execute when the component first mounts and is only meant for cleanup, not for returning values.
  return () => {
...
export const UserContext = createContext(null); // You can provide an optional default value inside createContext().
...
```
`Step 3` - Consume Context in Functional Components Using useContext
...
## 4. UseRef() hook
1. Accessing the Items of DOM elements. we can add a ref attribute to an element to access it directly in the DOM.  useRef can create a reference to a DOM element, allowing you to directly access and modify it. This is useful for things like focusing an input field, measuring element dimensions, or integrating with third-party libraries that require direct DOM access.
``` js
function App() {
...
2. React.memo() is a higher-order component (HOC) provided by React that memoizes functional components. It means that it caches the result of the component's rendering based on its props, and re-renders only when the props have changed.
3.  is used to optimize React components by preventing unnecessary re-renders when props don't change.

    ### Syntax
    ``` js
    // Define the Percentage component and memoize it
    const Percentage = React.memo(({ totalmarks }) => {
